...
DORKS: Dict[str, List[Dict[str, str]]] = {
    "Files Exposed (filetype)": [
        ...
        {"name": "XLS Exposed", "dork": 'site:{target} (filetype:xls OR filetype:xlsx OR filetype:csv)'},
        {"name": "Log Files Exposed", "dork": 'site:{target} (filetype:log OR filetype:sql OR filetype:env)'},
        {"name": "Backup file", "dork": 'site:{target} (filetype:bak OR filetype:old OR filetype:backup)'},
        {"name": "File konfigurasi", "dork": 'site:{target} (filetype:conf OR filetype:cfg OR filetype:ini)'},
    ],
    "Login/Admin Exposed": [
        {"name": "Halaman admin", "dork": 'site:{target} inurl:admin OR inurl:login OR inurl:wp-admin'},
        {"name": "Login form", "dork": 'site:{target} intitle:"login" OR intitle:"sign in"'},
        {"name": "Panel admin", "dork": 'site:{target} inurl:dashboard OR inurl:panel OR inurl:cpanel'},
    ],
    "Directory Exposed": [
    ...
    "Data Exposed": [
        ...
        {"name": "API Key Exposed", "dork": 'site:{target} "api_key" OR "apikey" OR "access_token"'},
        {"name": "Database Exposed", "dork": 'site:{target} inurl:db OR inurl:database OR inurl:phpmyadmin'},
    ],
    "Error/Debug Exposed": [
    ...
    "Tech Stack Detection": [
        ...
        {"name": "PHP framework", "dork": 'site:{target} inurl:laravel OR inurl:symfony OR inurl:yii'},
        {"name": "Server header", "dork": 'site:{target} inurl:"server-status" OR inurl:"server-info"'},
    ],
}
